...
import com.example.DataGenerator.enum.WeightedData
...
fun randomNumberInt(size: Int): BigInteger =
    List(size) { (1..9).random() }.joinToString("").toBigInteger()

fun randomIPAddress() =
    "${randomNumber(2)}.${randomNumber(2)}.${randomNumber(3)}.${randomNumber(3)}"

fun randomEmailSig() =
    Customer.values().random().customerName.replace("\\s".toRegex(), "").lowercase()
